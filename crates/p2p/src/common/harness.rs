...
impl KitsuneTestHarness {
    ...
    pub async fn create_agent(&mut self, name: &str) -> KAgent {
        self.legacy_host_api.create_agent(name).await
    }

    pub async fn get_agent(&self, name: &str) -> KAgent {
        self.legacy_host_api.get_agent(name).await
    }
